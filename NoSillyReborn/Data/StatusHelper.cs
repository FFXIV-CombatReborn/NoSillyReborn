...
public static class StatusHelper
{
    ...
    /// </returns>
    public static unsafe bool IsStatusCapped(IBattleChara battleChara)
    {
        if (battleChara == null)
            return false;
    ...
    public static bool PlayerWillStatusEndGCD(uint gcdCount = 0, float offset = 0, bool isFromSelf = true, params StatusID[] statusIDs)
    {
        ...
        return PlayerWillStatusEnd(PlayerData.GCDTime(gcdCount, offset), isFromSelf, statusIDs);
    }
    ...
    {
        return WillStatusEnd(battleChara, PlayerData.GCDTime(gcdCount, offset), isFromSelf, statusIDs);
    }
    ...
    public static bool PlayerHasApplyStatus(StatusID[] statusIDs)
    {
        ...
        if (PlayerData.InEffectTime && PlayerData.ApplyStatus.TryGetValue(Player.Object.GameObjectId, out var statusId))
        {
    ...
    public static bool HasApplyStatus(this IBattleChara battleChara, StatusID[] statusIDs)
    {
        ...
        if (PlayerData.InEffectTime && PlayerData.ApplyStatus.TryGetValue(battleChara.GameObjectId, out var statusId))
        {
            foreach (var s in statusIDs)
            {
                if ((uint)s == statusId)
    ...
    {
        var sheet = Plugin.GetSheet<Lumina.Excel.Sheets.Status>();
        if (sheet == null)
        {
            return string.Empty;
        }
